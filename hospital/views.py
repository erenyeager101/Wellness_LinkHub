...
def doctor_dashboard_view(request):
    ...
    patientdischarged = models.PatientDischargeDetails.objects.distinct().filter(
...
def detection_view(request):
    return render(request, 'hospital/detection.html')
...
def write_prescription(request: HttpRequest):
    # Check if the referrer is the doctor dashboard
    referrer = request.META.get('HTTP_REFERER', '')
    if 'doctor-dashboard_card' in referrer:
        # Generate a new Google Docs link for prescription
        ...
        try:
            ...
            return render(request, 'error.html',
                          {'message': 'Failed to create prescription document.', 'error': str(e)})

        # Extract the URL of the newly created document
        prescription_link = response.url

        # Generate a unique token for the prescription link
        prescription_token = str(uuid.uuid4())

        # Store the prescription link token in the session
        request.session['prescription_token'] = prescription_token

        # Store the prescription link in the session with the token as key
        request.session[prescription_token] = prescription_link

        # Store the prescription link in session variable for later use
        request.session['prescription_link'] = prescription_link

        # Redirect to the newly created document for editing
        return redirect(prescription_link)
    else:
        # Redirect to doctor dashboard or any other appropriate page
        return redirect('doctor_dashboard_cards')


def get_prescription_link(request: HttpRequest, token: str) -> HttpResponse:
    # Retrieve the prescription link using the token
    prescription_link = request.session.get(token)
    if prescription_link:
        return HttpResponse(f'Prescription Link: <a href="{prescription_link}">{prescription_link}</a>')
    else:
...
def patient_dashboard(request):
    prescription_link = request.session.get('prescription_link', None)
    return render(request, 'patient_dashboard.html', {'prescription_link': prescription_link})
...
def insurance_view(request):
    return render(request, 'newinsurance.html')
